...
function sendFullWindowList() {
    var windows = workspace.stackingOrder;
    var serialized = [];
    for (var i = 0; i < windows.length; i++) {
        serialized.push(JSON.parse(serializeWindow(windows[i])));
    }
    var jsonArray = JSON.stringify(serialized);
    callDBus(BRIDGE_SERVICE, BRIDGE_PATH, BRIDGE_IFACE, "UpdateWindowList", jsonArray);
}

workspace.windowAdded.connect(function(w) {
    var json = serializeWindow(w);
    callDBus(BRIDGE_SERVICE, BRIDGE_PATH, BRIDGE_IFACE, "NotifyWindowAdded", json);
});

workspace.windowRemoved.connect(function(w) {
    var windowId = w.internalId ? w.internalId.toString() : "";
    callDBus(BRIDGE_SERVICE, BRIDGE_PATH, BRIDGE_IFACE, "NotifyWindowRemoved", windowId);
});

// Send full window list on script load
sendFullWindowList();
